...
if $DRY_RUN; then
  echo -e "${YELLOW}[dry-run]${NC} go build -ldflags \"${LDFLAGS}\" ./cmd/vfs"
else
  go build -ldflags "$LDFLAGS" -o /dev/null ./cmd/vfs || fail "Build failed. Fix errors before releasing."
fi
ok "Build succeeded (${VERSION}, ${COMMIT}, ${BUILD_DATE})"
...
echo "    1. Build Linux binaries (amd64 + arm64)"
echo "    2. Create a GitHub Release with downloadable assets"
echo "    3. Check: https://github.com/TrNgTien/vfs/actions"
echo ""
echo "  Users can install with:"
echo ""
echo "    # Linux: download pre-built binary (no Go needed)"
echo "    curl -L https://github.com/TrNgTien/vfs/releases/latest/download/vfs-linux-amd64.tar.gz | tar xz"
echo "    sudo mv vfs /usr/local/bin/"
echo ""
echo "    # Any OS: build from source"
